...
def main():
    data_dir = '/n/holystore01/LABS/iaifi_lab/Lab/nswood/JetClass/Pythia'
    data_split = os.listdir(data_dir)
    all_output_dir = 'JetClass_gromov_delta_results_EMD_corrected'
    os.makedirs(all_output_dir, exist_ok=True)
    for cur_dir in data_split:
        # if 'train' in cur_dir or 'test' in cur_dir:
        #     continue
        print(f"Reading {cur_dir}")
        JetClass_data_path =os.path.join(data_dir, cur_dir)
        print('JetClass_data_path:', JetClass_data_path)
        JetClass_data_files = os.listdir(JetClass_data_path)
        JetClass_data_files = [os.path.join(JetClass_data_path,file) for file in JetClass_data_files if ('000.root' in file) or ('100.root' in file) or ('120.root' in file)]
        
        output_dir = os.path.join(all_output_dir, cur_dir.split('_')[0])
        os.makedirs(output_dir, exist_ok=True)
        for file in JetClass_data_files:
            # if 'HToWW2Q1L' not in file:
            #     continue
            ...
            with open(output_csv, mode='w', newline='') as file:
                ...
                writer.writerow(["index", 'jet_pt', 'jet_eta', 'jet_phi', 'jet_energy', 
                                                'jet_nparticles', 'jet_sdmass', 'jet_tau1', 
                                                'jet_tau2', 'jet_tau3', 'jet_tau4' ,"delta", "rel_delta", "c"])

            
            for i in range(num_jets):

                
                jet_pt, jet_eta, jet_phi, jet_energy, jet_nparticles, jet_sdmass, jet_tau1, jet_tau2, jet_tau3, jet_tau4 = x_jets[i]
                ...
                with open(output_csv, mode='a', newline='') as file:
                    writer = csv.writer(file)
                    writer.writerow([i, jet_pt, jet_eta, jet_phi, jet_energy, jet_nparticles, jet_sdmass, jet_tau1, jet_tau2, jet_tau3, jet_tau4, delta, rel_delta.item(), c.item()])
